...
// Shaft = additive revolution (same geometry as Groove but not subtracted).
export function ShaftMesh({ feature, allFeatures, highlighted, onClick }) {
  ...
  const geometry = useMemo(() => {
    if (!sketch) {
      console.warn(`[ShaftMesh] "${feature.Nom}" — sketch "${sketchNom}" introuvable.`);
      return null;
    }
    const points = buildLatheProfile(sketch);
    if (!points) {
      console.warn(`[ShaftMesh] "${feature.Nom}" — profil de révolution indéterminable.`);
      return null;
    }
    ...
    try {
      ...
      console.warn(`[ShaftMesh] "${feature.Nom}" — LatheGeometry échoué:`, e.message);
      return null;
    }
